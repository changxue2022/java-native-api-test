...
public class TestInsert_Error extends TestInsertUtil {
    ...
    public void testInsertTabletError1() throws IOException, IoTDBConnectionException, StatementExecutionException {
        // 准备环境和数据
        createTimeSeries();
        // 1、创建一个新的tablet实例
...
//                 处理null值
            ...
            if ("java.lang.ClassCastException".equals(e.getClass().getName())) {
                // 打印异常
                out.println("测试通过-预期错误:" + e);
            } else {
                // 若不是对应异常则测试失败
                throw new RuntimeException("测试失败-其他错误:" + e);
            }
        }
    }
     ...
     */
    @Test(priority = 10) // 测试执行的优先级为10
    public void testInsertAlignedTabletError1() throws IOException, IoTDBConnectionException, StatementExecutionException {
        // 准备环境和数据
...
//                 处理null值
     ...
     */
    @Test(priority = 10) // 测试执行的优先级为10
    public void testInsertTabletError2() throws IOException, IoTDBConnectionException, StatementExecutionException {
        // 准备环境和数据
        createTimeSeries();
// 1、创建一个新的tablet实例
        Tablet tablet = new Tablet(deviceId, schemaList, 10);
...
//                }
            ...
            out.println(e);
            if ("java.lang.NumberFormatException".equals(e.getClass().getName()) || "java.time.format.DateTimeParseException".equals(e.getClass().getName())) {
                // 打印异常
                out.println("测试通过-预期错误:" + e);
            } else {
                // 若不是对应异常则测试失败
...
//                }
    ...
    public void testInsertRecordInferenceError1() throws IoTDBConnectionException, StatementExecutionException, IOException {
        ...
        for (Iterator<Object[]> it = getSingleNormal(); it.hasNext(); ) {
            ...
            try {
                ...
                out.println(e);
                if ("org.apache.iotdb.rpc.StatementExecutionException".equals(e.getClass().getName())) {
                    // 打印异常
                    out.println("测试通过-预期错误:" + e);
                    // 退出程序
                    return;
    ...
    public void testTSError() throws IoTDBConnectionException, StatementExecutionException {
        ...
        try {
            ...
            if ("org.apache.iotdb.rpc.BatchExecutionException".equals(e.getClass().getName())) {
                // 打印异常
                out.println("测试通过：预期错误" + e.getMessage());
            } else {
                // 打印异常
                out.println("测试失败：其他错误" + e.getMessage());
                // 若不是对应异常则测试失败
            ...
            for (String path : paths) {
                // 判断时间序列是否存在
                System.out.println((session.checkTimeseriesExists(path)) ? "时间序列成功创建" : "时间序列创建失败");
            }
        }
    }
